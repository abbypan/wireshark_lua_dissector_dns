...
local dns = Proto("aadns","aaDNS Protocol")
...
local pf_trasaction_id      = ProtoField.new   ("Transaction ID", "aadns.trans_id", ftypes.UINT16)
local pf_flags              = ProtoField.new   ("Flags", "aadns.flags", ftypes.UINT16, nil, base.HEX)
local pf_num_questions      = ProtoField.uint16("aadns.num_questions", "Number of Questions")
local pf_num_answers        = ProtoField.uint16("aadns.num_answers", "Number of Answer RRs")
local pf_num_authority_rr   = ProtoField.uint16("aadns.num_authority_rr", "Number of Authority RRs")
local pf_num_additional_rr  = ProtoField.uint16("aadns.num_additional_rr", "Number of Additional RRs")
...
local pf_flag_response              = ProtoField.new   ("Response", "aadns.flags.response", ftypes.BOOLEAN, {"this is a response","this is a query"}, 16, 0x8000, "is the message a response?")
local pf_flag_opcode                = ProtoField.new   ("Opcode", "aadns.flags.opcode", ftypes.UINT16, nil, base.DEC, 0x7800, "operation code")
local pf_flag_authoritative         = ProtoField.new   ("Authoritative", "aadns.flags.authoritative", ftypes.BOOLEAN, nil, 16, 0x0400, "is the response authoritative?")
local pf_flag_truncated             = ProtoField.bool  ("aadns.flags.truncated", "Truncated", 16, nil, 0x0200, "is the message truncated?")
local pf_flag_recursion_desired     = ProtoField.bool  ("aadns.flags.recursion_desired", "Recursion desired", 16, {"yes","no"}, 0x0100, "do the query recursivley?")
local pf_flag_recursion_available   = ProtoField.bool  ("aadns.flags.recursion_available", "Recursion available", 16, nil, 0x0080, "does the server support recursion?")
local pf_flag_z                     = ProtoField.uint16("aadns.flags.z", "World War Z - Reserved for future use", base.HEX, nil, 0x0040, "when is it the future?")
local pf_flag_authenticated         = ProtoField.bool  ("aadns.flags.authenticated", "Authenticated", 16, {"yes","no"}, 0x0020, "did the server DNSSEC authenticate?")
local pf_flag_checking_disabled     = ProtoField.bool  ("aadns.flags.checking_disabled", "Checking disabled", 16, nil, 0x0010)
...
local pf_flag_rcode         = ProtoField.uint16("aadns.flags.rcode", "Response code", base.DEC, rcodes, 0x000F)
local pf_query              = ProtoField.new("Query", "aadns.query", ftypes.BYTES)
local pf_query_name         = ProtoField.new("Name", "aadns.query.name", ftypes.STRING)
local pf_query_name_len     = ProtoField.new("Name Length", "aadns.query.name.len", ftypes.UINT8)
local pf_query_label_count  = ProtoField.new("Label Count", "aadns.query.label.count", ftypes.UINT8)
local rrtypes = { [1] = "A (IPv4 host address)", [2] = "NS (authoritative name server)", [5]="CNAME", [28] = "AAAA (for geeks only)" }
local pf_query_type         = ProtoField.uint16("aadns.query.type", "Type", base.DEC, rrtypes)
...
local pf_query_class        = ProtoField.uint16("aadns.query.class", "Class", base.DEC, classes, nil, "keep it classy folks")
local pf_rr_ttl        = ProtoField.uint32("aadns.rr.ttl", "TTL")
local pf_rr_data_len  = ProtoField.uint16("aadns.rr.data_len", "[datalen]")
local pf_rr_data         = ProtoField.new("RRData", "aadns.rr.data", ftypes.STRING)
local pf_add_name  = ProtoField.new("Name", "aadns.additional.name", ftypes.UINT8)
local pf_add_type      = ProtoField.new   ("Type", "aadns.additional.type", ftypes.UINT16)
local pf_add_payload_size      = ProtoField.new   ("Size", "aadns.additional.payload.size", ftypes.UINT16)
local pf_add_high_rcode      = ProtoField.new   ("High Bits in Rcode", "aadns.additional.high.rcode", ftypes.UINT8)
local pf_add_edns_version      = ProtoField.new   ("EDNS Version", "aadns.additional.edns_version", ftypes.UINT8)
local pf_add_z      = ProtoField.new   ("Z", "aadns.additional.z", ftypes.UINT16)
local pf_add_data_len      = ProtoField.new   ("DataLen", "aadns.additional.data.len", ftypes.UINT16)
local pf_add_edns_data         = ProtoField.new("EDNSData", "aadns.edns.data", ftypes.STRING)
local pf_rr_data_ipv4 = ProtoField.new("RRData", "aadns.rr.data.ipv4", ftypes.IPv4)
...
dns.fields = { pf_trasaction_id, pf_flags,
    ...
    pf_rr_ttl, pf_rr_data_len, pf_rr_data, pf_rr_data_ipv4, 
    pf_add_name, pf_add_type, pf_add_payload_size, pf_add_high_rcode, 
    pf_add_edns_version, pf_add_z, pf_add_data_len, pf_add_edns_data
}
...
local ef_query     = ProtoExpert.new("aadns.query.expert", "DNS query message",
                                     expert.group.REQUEST_CODE, expert.severity.CHAT)
local ef_response  = ProtoExpert.new("aadns.response.expert", "DNS response message",
                                     expert.group.RESPONSE_CODE, expert.severity.CHAT)
local ef_ultimate  = ProtoExpert.new("aadns.response.ultimate.expert", "DNS answer to life, the universe, and everything",
                                     expert.group.COMMENTS_GROUP, expert.severity.NOTE)
-- some error expert info's
local ef_too_short = ProtoExpert.new("aadns.too_short.expert", "DNS message too short",
                                     expert.group.MALFORMED, expert.severity.ERROR)
local ef_bad_query = ProtoExpert.new("aadns.query.missing.expert", "DNS query missing or malformed",
...
local questions_field       = Field.new("aadns.num_questions")
local query_type_field      = Field.new("aadns.query.type")
local query_class_field     = Field.new("aadns.query.class")
local response_field        = Field.new("aadns.flags.response")
...
-- "aadns.flags.response" ProtoField to ftype.BOOLEAN way earlier when we created the
...
function dns.prefs_changed()
    ...
    if default_settings.port ~= dns.prefs.port then
        ...
        if default_settings.port ~= 0 then
            dprint("removing aadns from port",default_settings.port)
        ...
        if default_settings.port ~= 0 then
            dprint("adding aadns to port",default_settings.port)
...
function dns.dissector(tvbuf,pktinfo,root)
    dprint("dns.dissector called")

    -- set the protocol column to show our protocol name
    pktinfo.cols.protocol:set("aadns")
    ...
    local num_add = tvbuf:range(10,2):uint()
    dprint("-->> num add: " .. num_add )
    ...
    if num_queries > 0 then
        ...
        while num_queries > 0 and pktlen_remaining > 0 do
            ...
            dprint("-->> query: "..name..": type "..query_type_field().display ..", class "..query_class_field().display)
    ...
    num_arr = { num_answers, num_auth }
    tree_name = { "Answer", "Authority" }

    for i,n in ipairs(num_arr) do
        treeN = tree_name[i]
        dprint(treeN)
        dprint(i)
        dprint(n)
        if n > 0 then
            -- let's create a sub-tree, using a plain text description (not a field from the packet)
            local ans_tree = tree:add(treeN)
            local pktlen_remaining = pktlen - pos
            while n > 0 do

                local name, name_len = getRRName(tvbuf, tvbuf:range(pos,pktlen_remaining), pktlen)
                local a_tree = ans_tree:add("RR")
                a_tree:add(pf_query_name, name)

                pos = pos + name_len

                pktinfo.cols.info:append(treeN..": "..name)

                local rtype = tvbuf:range(pos, 2):uint()
                a_tree:add(pf_query_type, rtype)
                a_tree:add(pf_query_class, tvbuf:range(pos + 2, 2))
                pos = pos + 4

                a_tree:add(pf_rr_ttl, tvbuf:range(pos, 4))
                pos = pos + 4

                local rr_dlen = tvbuf:range(pos,2):uint()
                a_tree:add(pf_rr_data_len, tvbuf:range(pos, 2))
                pos = pos + 2

                local rr_data = tvbuf:range(pos, rr_dlen)
                if rtype==1 then
                    a_tree:add(pf_rr_data_ipv4, rr_data)
                elseif rtype==2 then
                    dprint("NS")
                    --local tname, tlen = getRRName(tvbuf, tvbuf:range(pos,rr_dlen), pktlen)
                    --dprint(tname)
                    --a_tree:add(pf_rr_data, tname)
                    a_tree:add(pf_rr_data, rr_data)
                elseif rtype==5 then
                    dprint("CNAME") 
                    --local tname, tlen = getRRName(tvbuf, tvbuf:range(pos,rr_dlen), pktlen)
                    --dprint(tname)
    ...
    --if num_auth > 0 then
        --local auth_tree = tree:add("Authority")
        --local pktlen_remaining = pktlen - pos
        --while num_auth > 0 do

            --local name, name_len = getRRName(tvbuf, tvbuf:range(pos,pktlen_remaining), pktlen)
            --local a_tree = auth_tree:add("RR")
            --a_tree:add(pf_query_name, name)

            --pos = pos + name_len

            --pktinfo.cols.info:append(" Authority: "..name)

            --a_tree:add(pf_query_type, tvbuf:range(pos, 2))
            --a_tree:add(pf_query_class, tvbuf:range(pos+2, 2))
            --pos = pos + 4
            ...
            --local rr_data = tvbuf:range(pos, rr_dlen)
            --a_tree:add(pf_rr_data, tvbuf:range(pos, rr_dlen))
            --pos = pos + rr_dlen
    ...
    if num_add > 0 then
        -- let's create a sub-tree, using a plain text description (not a field from the packet)
        local add_tree = tree:add("Additional")
        local pktlen_remaining = pktlen - pos
        while num_add > 0 do

            --pktinfo.cols.info:append(" Authority: "..name)

            add_tree:add(pf_add_name, tvbuf:range(pos, 1))
            add_tree:add(pf_add_type, tvbuf:range(pos+1, 2))
            add_tree:add(pf_add_payload_size, tvbuf:range(pos+3, 2))
            add_tree:add(pf_add_high_rcode, tvbuf:range(pos+5, 1))
            add_tree:add(pf_add_edns_version, tvbuf:range(pos+6, 1))
            add_tree:add(pf_add_z, tvbuf:range(pos+7, 2))

            local dlen = tvbuf:range(pos+9,2):uint()
            add_tree:add(pf_add_data_len, dlen)

            local rr_data = tvbuf:range(pos+11, dlen)
            add_tree:add(pf_add_edns_data, rr_data)
            pos=pos+11+dlen
    ...
    dprint("dns.dissector returning",pos)
...
local function heur_dissect_dns(tvbuf,pktinfo,root)
    dprint("heur_dissect_dns called")
    ...
    dprint("heur_dissect_dns checking questions/answers")
    ...
    dprint("heur_dissect_dns: everything looks good calling the real dissector")
...
getRRName = function (tvbuf, tvbr, pktlen)
    local label_count = 0
    local name = ""
    ...
    local label_len = barray:get_index(pos)
    if label_len >= 192 then
        local npos =barray:get_index(1)
        npos = (label_len-192)*256 + npos
        local xlabel_count, xname, xname_len = getQueryName(tvbuf:range(npos,pktlen - npos))
        name = name .. xname
        return name, 2
    end

    local xlabel_count, xname, xname_len = getQueryName(tvbr)
    return xname, xname_len
